...
null = bayesglm(doms_IsDomesticated~1, data = mice_df, family = "binomial", scaled = T)
full = bayesglm(doms_IsDomesticated~., data = mice_df, family = "binomial", scaled = T)

# do backward selection:
options(warn=-1)
step(null, scope = list(upper=full), data=mice_df, direction="both", trace = 0)
```
...
formula = doms_IsDomesticated ~ cont_X5.3_NeonateBodyMass_g + 
    cont_X26.2_GR_MaxLat_dd + cont_X26.3_GR_MinLat_dd + disc_X12.1_HabitatBreadth + 
    cont_X28.1_Precip_Mean_mm + cont_X30.1_AET_Mean_mm
