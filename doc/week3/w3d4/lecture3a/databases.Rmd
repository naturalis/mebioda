...
for ( trait in colnames(traits_tr8@results) ) {
  for ( spec in species ) {
    results[ spec, trait ] <- traits_tr8@results[ spec, trait ]
  }
}
```
...
for (sp in species) {
  ...
  for ( trait in eol_traits[["graph"]]$predicate ) {
    results[ sp, trait ] <- ( eol_traits[["graph"]][ eol_traits[["graph"]]$predicate == trait, ]$value )[1]
  }
}
