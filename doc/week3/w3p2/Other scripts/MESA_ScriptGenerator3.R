...
#SEED_LIST = c(4985) #seq(from = 4800, to = 5000, by=1)
...
TIME_EXTEVENT = 450 #300
TIME_IMMPOSTEXT = TIME_EXTEVENT+1
TIME_LIMIT = 1000 #600

TIME_EXTEVENT_INDEX = which(TIME_LIST==TIME_EXTEVENT)
...
for (z in 1:SLSIZE)
{
  ...
  for (x in 1:TIME_EXTEVENT_INDEX) {
    writeString = "*****"
    writeLines(writeString,con=OUTFILE,sep="\n")
    writeString = RUN_EPOCH_OPT
    writeLines(writeString,con=OUTFILE,sep="\n")
  ...
  for (x in (TIME_EXTEVENT_INDEX+1):TIME_LIMIT_INDEX) {
    writeString = "*****"
    writeLines(writeString,con=OUTFILE,sep="\n")
    writeString = RUN_EPOCH_OPT
    writeLines(writeString,con=OUTFILE,sep="\n")
